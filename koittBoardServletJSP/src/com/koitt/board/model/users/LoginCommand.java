...
public class LoginCommand implements Command {
	...
	public String execute(HttpServletRequest req, HttpServletResponse resp)
		...
		// 포워딩할 JSP 주소(로그인 성공 시, 게시판 목록으로 이동)
		String page = "./BoardServlet?cmd=CMD_LIST";
		
		// 이메일과 패스워드를 클라이언트로부터 전달받는다.
		String email = req.getParameter("email");
		String password = req.getParameter("password");
		
		// 사용자 존재여부와 비밀번호 체크하기 위해 UsersDao 객체를 생성
		...
		if(matches) {
			...
			// 현재 로그인한 사용자의 email값을 세션에 존재한다면 로그인 된 상태로 간주
			ssesion.setAttribute("email", email);
